...
2. Connect to the DendroPulse database using `get_dendropulse()`
3. Import all tables locally using `get_db()`
4. Save a snapshot of the database to disk
5. Reload an existing database snapshot using `get_saveddb()`
6. Combine several tables into a single flat dataset
7. Build a daily summary and visualize tree growth patterns
...
To load the different packages
...
db <- get_saveddb(dir = "c:/temp/dendropulse_copies/")
```
